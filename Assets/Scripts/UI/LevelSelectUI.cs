...
namespace RunAndGun
{
    public class LevelSelectUI : MonoBehaviour
    {
        ...
        private void Awake() { if (backButton != null) backButton.onClick.AddListener(OnBackClicked); }
        private void OnEnable() { BuildGrid(); ClearPreview(); }
        private void OnDestroy() { if (backButton != null) backButton.onClick.RemoveListener(OnBackClicked); }

        private void BuildGrid()
        {
            if (spawnedButtons != null) { for (int i = 0; i < spawnedButtons.Length; i++) { if (spawnedButtons[i] != null) Destroy(spawnedButtons[i].gameObject); } }
            int levelCount = GameManager.Instance != null ? GameManager.Instance.LevelCount : (levels != null ? levels.Length : 0);
            ...
            for (int i = 0; i < levelCount; i++)
            {
                ...
                string displayName = (levels != null && i < levels.Length) ? levels[i].displayName : $"Level {i + 1}";
        ...
        private void OnLevelButtonClicked(int levelIndex)
        {
            ...
            if (!unlocked) { if (lockedSfx != null && AudioManager.Instance != null) AudioManager.Instance.PlaySFX(lockedSfx); return; }
            PlayClickSound();
            ShowPreview(levelIndex);
            if (GameManager.Instance != null) GameManager.Instance.LoadLevel(levelIndex);
        }
        ...
        private void ShowPreview(int levelIndex)
        {
            ...
            if (previewImage != null && entry.previewSprite != null) { previewImage.sprite = entry.previewSprite; previewImage.enabled = true; }
            if (previewLevelName != null) previewLevelName.text = entry.displayName;
            if (previewHighScore != null && ScoreManager.Instance != null) previewHighScore.text = $"Best: {ScoreManager.Instance.GetHighScore(levelIndex)}";
        }

        private void ClearPreview()
        {
            if (previewImage != null) previewImage.enabled = false;
            if (previewLevelName != null) previewLevelName.text = string.Empty;
            if (previewHighScore != null) previewHighScore.text = string.Empty;
        }

        private void OnBackClicked() { PlayClickSound(); gameObject.SetActive(false); }
    ...
    public class LevelButtonUI : MonoBehaviour
    {
        ...
        public Button Button { get { if (button == null) button = GetComponent<Button>(); return button; } }

        public void Setup(int levelIndex, string displayName, bool unlocked, int starCount, Sprite starFilled, Sprite starEmpty)
        {
            ...
            if (starImages != null)
            {
                for (int i = 0; i < starImages.Length; i++)
                {
                    ...
                    if (filled && starFilled != null) starImages[i].sprite = starFilled;
                    else if (!filled && starEmpty != null) starImages[i].sprite = starEmpty;
