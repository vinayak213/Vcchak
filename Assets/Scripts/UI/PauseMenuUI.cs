...
namespace RunAndGun
{
    public class PauseMenuUI : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (InputManager.Instance != null) pauseRequested = InputManager.Instance.PauseDown;
            else pauseRequested = Input.GetKeyDown(KeyCode.Escape);
            if (pauseRequested) { if (isPaused) Resume(); else Pause(); }
        }
        ...
        public void Resume()
        {
            PlayClickSound(); isPaused = false; Time.timeScale = 1f;
            if (pausePanel != null) pausePanel.SetActive(false);
            if (settingsPanel != null) settingsPanel.SetActive(false);
            if (GameManager.Instance != null) GameManager.Instance.SetState(GameState.Playing);
        }

        private void Restart() { PlayClickSound(); isPaused = false; Time.timeScale = 1f; if (GameManager.Instance != null) GameManager.Instance.RestartLevel(); }
        private void ReturnToMainMenu() { PlayClickSound(); isPaused = false; Time.timeScale = 1f; if (GameManager.Instance != null) GameManager.Instance.ReturnToMainMenu(); }

        private void OpenSettings() { PlayClickSound(); if (settingsPanel != null) settingsPanel.SetActive(true); SyncSettingsUI(); }
        private void CloseSettings() { PlayClickSound(); if (settingsPanel != null) settingsPanel.SetActive(false); }
        ...
        private void SyncSettingsUI()
        {
            ...
            if (muteToggle != null) muteToggle.SetIsOnWithoutNotify(AudioManager.Instance.IsMuted);
        }
        ...
        private void OnMuteToggled(bool muted) { if (AudioManager.Instance != null) AudioManager.Instance.SetMuted(muted); }
